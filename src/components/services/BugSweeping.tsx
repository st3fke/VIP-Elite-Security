...
export function BugSweeping() {
  ...
  const features = t('servicesDetails.bugSweeping.features', { returnObjects: true }) as string[];
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-4xl font-bold text-white">{t('servicesDetails.bugSweeping.title')}</h2>
            </div>
            <p className="text-gray-400 text-lg mb-6">{t('servicesDetails.bugSweeping.description1')}</p>
            <p className="text-gray-400 text-lg mb-6">{t('servicesDetails.bugSweeping.description2')}</p>
